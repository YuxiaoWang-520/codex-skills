...
这个 skill 围绕一个核心判断来设计：**对话是一座被反复浪费的知识金矿**。每次 session 中用户教给 agent 的东西，理应被系统化地留下来。

| 设计决策 | 为什么这样做 | 防的是什么问题 |
| --- | --- | --- |
| 只有两个命令（`/learn` + `/learn-review`） | 最小认知负担 | 命令太多记不住，用户不愿意用 |
| 用 Markdown 而不是 YAML/JSON | 人可读、可直接编辑、可 git 管理 | 知识变成黑盒，用户无法检查和修正 |
| 三档强度（`weak→medium→strong`） | 简单但有效的验证模型 | 浮点数伪精确（0.47 vs 0.52 没有实际区别） |
| 分 project/global 两级 | 项目经验不污染其他项目 | React 项目的习惯跑到 Python 项目里 |
| 半自动提升（建议+确认） | 用户确认后再全局化 | 错误知识污染所有项目，误提升代价高 |
| 质量门控（Save/Merge/Skip） | 保存前过滤噪音 | 知识目录退化成垃圾场 |

#### 四种知识类型（按价值排序）

1. **Corrections（纠正）** — 用户纠正过 agent 的做法，最高价值。因为这代表 agent 犯过的错，下次绝不能再犯。
2. **Patterns（模式）** — 反复出现的工作流或编码惯例。比如”在这个项目里，永远先跑 lint 再 commit”。
3. **Facts（事实）** — 项目/环境特定的真实信息。比如”CI 用的是 Node 20”、”部署需要 staging 审批”。
4. **Preferences（偏好）** — 用户的个人风格偏好。比如”回答简洁”、”注释用中文”。

#### 强度演化模型

这是 learn 最有匠心的设计之一。知识不是一经保存就永远有效，它需要被验证、被强化、也可能被推翻：

```
新知识诞生 → strength: weak, confirmed: 0
  ↓ 被应用一次且未被用户否定
...
strength: strong, confirmed: 4+
  ↓ 用户明确说”对，就是这样”
strength: strong（直接跳级）
```

用户说”不对”→ 降级或删除。没有自动衰减 — 知识不会因为没被用到就消失，过时的知识由 `/learn-review` 人工清理。

这比浮点置信度更实用：`weak/medium/strong` 三档足以区分”初步观察”和”反复验证”，不需要让系统纠结 0.47 和 0.52 的区别。
...
为什么不自动提升？因为误提升的代价（污染全局）远大于漏提升的代价（下次再教一遍）。

#### 用户体感设计

“越用越聪明”不能只是技术上做到了，用户必须**感受到**：

- 每次 `/learn`，清晰列出学了什么、跳过了什么、为什么
- 当 agent 因为已学知识做出不同决策时，主动说明原因：
  > “使用真实数据库进行集成测试（learned: 不要 mock 数据库）”
